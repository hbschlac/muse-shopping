/**
 * Populate Scoop with sample data matching Nordstrom's assortment
...
async function populateScoopData() {
  console.log('🏬 Populating Scoop with sample data...');
  ...
  for (const product of sampleProducts) {
    try {
      ...
      const productId = `SCOOP-${product.brand.replace(/[^A-Z0-9]/g, '')}-${productSlug}`;
      const imageUrl = `https://images.scoop.com/is/image/BLM/${productId}`;
      const productUrl = `https://www.scoop.com/shop/product/${productSlug}`;

      await pool.query(`
        INSERT INTO scoop_products (
  ...
  await pool.query(`
    INSERT INTO scoop_inventory_snapshots (
  ...
  console.log('\n✨ Scoop data population complete!');
  console.log(`📊 Summary:`);
