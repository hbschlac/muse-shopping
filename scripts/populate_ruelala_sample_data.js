/**
 * Populate RueLaLa with sample data matching Nordstrom's assortment
...
async function populateRueLaLaData() {
  console.log('🏬 Populating RueLaLa with sample data...');
  ...
  for (const product of sampleProducts) {
    try {
      ...
      const productId = `RLL-${product.brand.replace(/[^A-Z0-9]/g, '')}-${productSlug}`;
      const imageUrl = `https://images.ruelala.com/is/image/BLM/${productId}`;
      const productUrl = `https://www.ruelala.com/shop/product/${productSlug}`;

      await pool.query(`
        INSERT INTO ruelala_products (
  ...
  await pool.query(`
    INSERT INTO ruelala_inventory_snapshots (
  ...
  console.log('\n✨ RueLaLa data population complete!');
  console.log(`📊 Summary:`);
