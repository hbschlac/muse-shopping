  ...
  useEffect(() => {
    async function handleCallback() {
      try {
        ...
        // Get retailer ID from session storage
        const retailerId = sessionStorage.getItem('retailer_auth_id');
        if (!retailerId) {
          setStatus('error');
        ...
        setTimeout(() => {
          router.push('/settings/retailers');
        }, 2000);
      } catch (err) {
        setStatus('error');
        setErrorMessage(
          err instanceof Error
            ? err.message
            : 'Failed to connect retailer account'
        );
      }
    }
      ...
      <div className="max-w-md w-full bg-white rounded-[16px] shadow-lg p-8 text-center">
        {status === 'loading' && (
          <>
            ...
            <p className="text-gray-600">
              Please wait while we connect your retailer account...
            </p>
          </>
        )}
        ...
        {status === 'success' && (
          <>
            ...
            <p className="text-gray-600">
              Your retailer account has been successfully connected.
            </p>
        ...
        {status === 'error' && (
          <>
            ...
            <button
              onClick={() => router.push('/settings/retailers')}
