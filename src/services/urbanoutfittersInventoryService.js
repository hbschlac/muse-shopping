/**
 * Urban Outfitters Inventory Scraping Service
 ...
 *
 * Recommended: Contact Urban Outfitters for API access or explicit permission for research
 */
...
class UrbanOutfittersInventoryService {
  constructor() {
    this.baseUrl = 'https://www.urbanoutfitters.com';
  ...
  async scrapeInventory() {
    ...
    logger.info('[Urban Outfitters] Starting inventory scrape');
    ...
    try {
      ...
      logger.info(`[Urban Outfitters] Found ${categoryUrls.length} category URLs`);
      ...
      for (const categoryUrl of categoryUrls) {
        try {
          ...
          for (const product of products) {
            try {
              ...
              if (stats.totalProducts >= this.maxProductsPerRun) {
                logger.info(`[Urban Outfitters] Reached max products limit: ${this.maxProductsPerRun}`);
                break;
              }
            } catch (error) {
              stats.errors++;
              stats.errorLog.push(`Product ${product.productId}: ${error.message}`);
              logger.error(`[Urban Outfitters] Error saving product:`, error);
            }
          }
          ...
          logger.error(`[Urban Outfitters] Error scraping category ${categoryUrl}:`, error);
        }
      }
      ...
      logger.info(`[Urban Outfitters] Scrape completed. Products: ${stats.totalProducts}, Errors: ${stats.errors}`);
      ...
      logger.error('[Urban Outfitters] Fatal scraping error:', error);
  ...
  async getCategoryUrls(page) {
    try {
      ...
      try {
        ...
        logger.warn('[Urban Outfitters] Product cards did not load in time');
      }

      const urls = await page.evaluate((baseUrl) => {
        const links = [];
        const selectors = [
          'a[href*="/womens-clothes"]',
          'a[href*="/womens"]',
          'nav a[href*="clothes"]',
          '.category-link'
        ];
        ...
        selectors.forEach(selector => {
          ...
          elements.forEach(link => {
            ...
            if (href && href.includes('womens') && !links.includes(href)) {
      ...
      logger.info(`[Urban Outfitters] Found ${urls.length} potential category URLs`);
      ...
      // Fallback categories
      return [
        `${this.baseUrl}/womens-clothes`,
        `${this.baseUrl}/womens-clothes/dresses`,
        `${this.baseUrl}/womens-clothes/tops`,
        `${this.baseUrl}/womens-clothes/pants`,
        `${this.baseUrl}/womens-clothes/jackets-coats`
      ];

    } catch (error) {
      logger.error('[Urban Outfitters] Error getting category URLs:', error);
      return [
        `${this.baseUrl}/womens-clothes`,
        `${this.baseUrl}/womens-clothes/dresses`,
        `${this.baseUrl}/womens-clothes/tops`
  ...
  async scrapeCategory(page, categoryUrl) {
    ...
    try {
      logger.info(`[Urban Outfitters] Scraping category: ${categoryUrl}`);
      ...
      try {
        ...
        logger.warn('[Urban Outfitters] Network did not idle, continuing anyway');
      }
      ...
      const productData = await page.evaluate((baseUrl) => {
        ...
        productCards.forEach((card, index) => {
          try {
            ...
            let brandName = 'Urban Outfitters';
  ...
  async saveProduct(product) {
    ...
    try {
      ...
      const productResult = await client.query(`
        INSERT INTO urbanoutfitters_products (
      ...
      `, [
        product.productId,
        product.productName,
        product.brandName || 'Urban Outfitters',
        product.price,
      ...
      if (product.price) {
        await client.query(`
          INSERT INTO urbanoutfitters_price_history (product_id, price, was_on_sale, sale_price, recorded_at)
          VALUES ($1, $2, $3, $4, NOW())
  ...
  async saveSnapshot(date, stats, duration, status = 'success', errorLog = null) {
    try {
      ...
      await client.query(`
        INSERT INTO urbanoutfitters_inventory_snapshots (
      ...
      logger.error('[Urban Outfitters] Error saving snapshot:', error);
    }
  }
  ...
  async getPriceHistory(productId) {
    ...
    try {
      const result = await client.query(`
        ...
        FROM urbanoutfitters_price_history
        WHERE product_id = $1
...
module.exports = new UrbanOutfittersInventoryService();
