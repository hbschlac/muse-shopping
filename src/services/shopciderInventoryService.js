/**
 * Shop Cider Inventory Scraping Service
 * Purpose: Academic research - tracking Shop Cider fashion inventory
 *
 * IMPORTANT: This service is designed for academic research purposes.
 * Users must ensure compliance with:
 * - Shop Cider's Terms of Service
 * - Shop Cider's robots.txt
 * - Applicable data protection regulations
 * - Academic institution's research ethics guidelines
 *
 * Recommended: Contact Shop Cider for API access or explicit permission for research
 */
...
class ShopciderInventoryService {
  constructor() {
    this.baseUrl = 'https://shopcider.com';
  ...
  async scrapeInventory() {
    ...
    logger.info('[Shopcider] Starting inventory scrape');
    ...
    try {
      ...
      logger.info(`[Shopcider] Found ${collectionUrls.length} collection URLs`);
      ...
      for (const collectionUrl of collectionUrls) {
        try {
          ...
          for (const product of products) {
            try {
              ...
              if (stats.totalProducts >= this.maxProductsPerRun) {
                logger.info(`[Shopcider] Reached max products limit: ${this.maxProductsPerRun}`);
                break;
              }
            } catch (error) {
              stats.errors++;
              stats.errorLog.push(`Product ${product.productId}: ${error.message}`);
              logger.error(`[Shopcider] Error saving product:`, error);
            }
          }
          ...
          logger.error(`[Shopcider] Error scraping collection ${collectionUrl}:`, error);
        }
      }
      ...
      logger.info(`[Shopcider] Scrape completed. Products: ${stats.totalProducts}, Errors: ${stats.errors}`);
      ...
      logger.error('[Shopcider] Fatal scraping error:', error);
  ...
  async getCollectionUrls(page) {
    try {
      ...
      try {
        ...
        logger.warn('[Shopcider] Product cards did not load in time');
      }
      ...
      logger.info(`[Shopcider] Found ${urls.length} potential collection URLs`);
      ...
      logger.error('[Shopcider] Error getting collection URLs:', error);
      // Return fallback
      return [this.shopUrl];
    }
  }
  ...
  async scrapeCollection(page, collectionUrl) {
    ...
    try {
      logger.info(`[Shopcider] Scraping collection: ${collectionUrl}`);
      ...
      try {
        ...
        logger.warn('[Shopcider] Network did not idle, continuing anyway');
      }
      ...
      logger.info(`[Shopcider] Page info: ${JSON.stringify(pageInfo)}`);
      ...
      const productData = await page.evaluate(() => {
        ...
        productCards.forEach((card, index) => {
          try {
            ...
            // Extract brand (Shop Cider may not have explicit brand fields)
            const brandSelectors = ['.brand', '.vendor', '[itemprop="brand"]', 'span.brand'];
            let brandName = 'Shop Cider'; // Default brand
            ...
            if (imageUrl && !imageUrl.startsWith('http')) {
              imageUrl = imageUrl.startsWith('//') ? 'https:' + imageUrl : 'https://shopcider.com' + imageUrl;
            }
            ...
            if (productUrl && !productUrl.startsWith('http')) {
              productUrl = 'https://shopcider.com' + productUrl;
            }
  ...
  async saveProduct(product) {
    ...
    try {
      ...
      const productResult = await client.query(`
        INSERT INTO shopcider_products (
      ...
      if (product.price) {
        await client.query(`
          INSERT INTO shopcider_price_history (product_id, price, recorded_at)
          VALUES ($1, $2, NOW())
  ...
  async saveSnapshot(date, stats, duration, status = 'success', errorLog = null) {
    try {
      ...
      await client.query(`
        INSERT INTO shopcider_inventory_snapshots (
      ...
      logger.error('[Shopcider] Error saving snapshot:', error);
    }
  }
...
module.exports = new ShopciderInventoryService();
