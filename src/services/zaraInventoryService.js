/**
 * Zara Inventory Scraping Service
 ...
 *
 * Recommended: Contact Zara for API access or explicit permission for research
 */
...
class ZaraInventoryService {
  constructor() {
    this.baseUrl = 'https://www.zara.com';
    this.womensCategoryUrl = `${this.baseUrl}/us/en/woman-l1030.html`;
  ...
  async scrapeInventory() {
    ...
    logger.info('[Zara] Starting inventory scrape');
    ...
    try {
      ...
      logger.info(`[Zara] Found ${categoryUrls.length} category URLs`);
      ...
      for (const categoryUrl of categoryUrls) {
        try {
          ...
          for (const product of products) {
            try {
              ...
              if (stats.totalProducts >= this.maxProductsPerRun) {
                logger.info(`[Zara] Reached max products limit: ${this.maxProductsPerRun}`);
                break;
              }
            } catch (error) {
              stats.errors++;
              stats.errorLog.push(`Product ${product.productId}: ${error.message}`);
              logger.error(`[Zara] Error saving product:`, error);
            }
          }
          ...
          logger.error(`[Zara] Error scraping category ${categoryUrl}:`, error);
        }
      }
      ...
      logger.info(`[Zara] Scrape completed. Products: ${stats.totalProducts}, Errors: ${stats.errors}`);
      ...
      logger.error('[Zara] Fatal scraping error:', error);
  ...
  async getCategoryUrls(page) {
    try {
      ...
      try {
        ...
        logger.warn('[Zara] Product cards did not load in time');
      }
      ...
      const urls = await page.evaluate((baseUrl) => {
        ...
        selectors.forEach(selector => {
          ...
          elements.forEach(link => {
            ...
            if (href && href.includes('woman') && !links.includes(href)) {
      ...
      logger.info(`[Zara] Found ${urls.length} potential category URLs`);
      ...
      // Fallback categories
      return [
        `${this.baseUrl}/us/en/woman-l1030.html`,
        `${this.baseUrl}/us/en/woman-dresses-l2002.html`,
        `${this.baseUrl}/us/en/woman-shirts-l2003.html`,
        `${this.baseUrl}/us/en/woman-jeans-l2005.html`,
        `${this.baseUrl}/us/en/woman-jackets-l2001.html`
      ];

    } catch (error) {
      logger.error('[Zara] Error getting category URLs:', error);
      return [
        `${this.baseUrl}/us/en/woman-l1030.html`,
        `${this.baseUrl}/us/en/woman-dresses-l2002.html`,
        `${this.baseUrl}/us/en/woman-shirts-l2003.html`
  ...
  async scrapeCategory(page, categoryUrl) {
    ...
    try {
      logger.info(`[Zara] Scraping category: ${categoryUrl}`);
      ...
      try {
        ...
        logger.warn('[Zara] Network did not idle, continuing anyway');
      }
  ...
  async saveProduct(product) {
    ...
    try {
      ...
      if (product.price) {
        await client.query(`
          INSERT INTO zara_price_history (product_id, price, was_on_sale, sale_price, recorded_at)
          VALUES ($1, $2, $3, $4, NOW())
  ...
  async saveSnapshot(date, stats, duration, status = 'success', errorLog = null) {
    try {
      ...
      await client.query(`
        INSERT INTO zara_inventory_snapshots (
      ...
      logger.error('[Zara] Error saving snapshot:', error);
    }
  }
...
module.exports = new ZaraInventoryService();
