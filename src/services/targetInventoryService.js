/**
 * Target Inventory Scraping Service
 ...
 *
 * Recommended: Contact Target for API access or explicit permission for research
 */
...
class TargetInventoryService {
  constructor() {
    this.baseUrl = 'https://www.target.com';
    this.womensCategoryUrl = `${this.baseUrl}/c/women-s-clothing/-/N-5xtd6`;
  ...
  async scrapeInventory() {
    ...
    logger.info('[Target] Starting inventory scrape');
    ...
    try {
      ...
      logger.info(`[Target] Found ${categoryUrls.length} category URLs`);
      ...
      for (const categoryUrl of categoryUrls) {
        try {
          ...
          for (const product of products) {
            try {
              ...
              if (stats.totalProducts >= this.maxProductsPerRun) {
                logger.info(`[Target] Reached max products limit: ${this.maxProductsPerRun}`);
                break;
              }
            } catch (error) {
              stats.errors++;
              stats.errorLog.push(`Product ${product.productId}: ${error.message}`);
              logger.error(`[Target] Error saving product:`, error);
            }
          }
          ...
          logger.error(`[Target] Error scraping category ${categoryUrl}:`, error);
        }
      }
      ...
      logger.info(`[Target] Scrape completed. Products: ${stats.totalProducts}, Errors: ${stats.errors}`);
      ...
      logger.error('[Target] Fatal scraping error:', error);
  ...
  async getCategoryUrls(page) {
    try {
      ...
      try {
        ...
        logger.warn('[Target] Product cards did not load in time');
      }
      ...
      const urls = await page.evaluate((baseUrl) => {
        ...
        selectors.forEach(selector => {
          ...
          elements.forEach(link => {
            ...
            if (href && (href.includes('women') || href.includes('/c/')) && !links.includes(href)) {
      ...
      logger.info(`[Target] Found ${urls.length} potential category URLs`);
      ...
      // Fallback categories
      return [
        `${this.baseUrl}/c/women-s-clothing/-/N-5xtd6`,
        `${this.baseUrl}/c/women-s-dresses/-/N-5xtf0`,
        `${this.baseUrl}/c/women-s-tops-t-shirts/-/N-5xtu8`,
        `${this.baseUrl}/c/women-s-pants-leggings/-/N-5xtuf`,
        `${this.baseUrl}/c/women-s-coats-jackets/-/N-5xtvk`
      ];

    } catch (error) {
      logger.error('[Target] Error getting category URLs:', error);
      return [
        `${this.baseUrl}/c/women-s-clothing/-/N-5xtd6`,
        `${this.baseUrl}/c/women-s-dresses/-/N-5xtf0`,
        `${this.baseUrl}/c/women-s-tops-t-shirts/-/N-5xtu8`
  ...
  async scrapeCategory(page, categoryUrl) {
    ...
    try {
      logger.info(`[Target] Scraping category: ${categoryUrl}`);
      ...
      try {
        ...
        logger.warn('[Target] Network did not idle, continuing anyway');
      }
  ...
  async saveProduct(product) {
    ...
    try {
      ...
      if (product.price) {
        await client.query(`
          INSERT INTO target_price_history (product_id, price, was_on_sale, sale_price, recorded_at)
          VALUES ($1, $2, $3, $4, NOW())
  ...
  async saveSnapshot(date, stats, duration, status = 'success', errorLog = null) {
    try {
      ...
      await client.query(`
        INSERT INTO target_inventory_snapshots (
      ...
      logger.error('[Target] Error saving snapshot:', error);
    }
  }
...
module.exports = new TargetInventoryService();
