/**
 * Dynamite Inventory Scraping Service
 ...
 *
 * Recommended: Contact Dynamite for API access or explicit permission for research
 */
...
class DynamiteInventoryService {
  constructor() {
    this.baseUrl = 'https://www.dynamiteclothing.com';
    this.womensCategoryUrl = `${this.baseUrl}/us/clothing/c/22`;
  ...
  async scrapeInventory() {
    ...
    logger.info('[Dynamite] Starting inventory scrape');
    ...
    try {
      ...
      logger.info(`[Dynamite] Found ${categoryUrls.length} category URLs`);
      ...
      for (const categoryUrl of categoryUrls) {
        try {
          ...
          for (const product of products) {
            try {
              ...
              if (stats.totalProducts >= this.maxProductsPerRun) {
                logger.info(`[Dynamite] Reached max products limit: ${this.maxProductsPerRun}`);
                break;
              }
            } catch (error) {
              stats.errors++;
              stats.errorLog.push(`Product ${product.productId}: ${error.message}`);
              logger.error(`[Dynamite] Error saving product:`, error);
            }
          }
          ...
          logger.error(`[Dynamite] Error scraping category ${categoryUrl}:`, error);
        }
      }
      ...
      logger.info(`[Dynamite] Scrape completed. Products: ${stats.totalProducts}, Errors: ${stats.errors}`);
      ...
      logger.error('[Dynamite] Fatal scraping error:', error);
  ...
  async getCategoryUrls(page) {
    try {
      ...
      try {
        ...
        logger.warn('[Dynamite] Product cards did not load in time');
      }
      ...
      logger.info(`[Dynamite] Found ${urls.length} potential category URLs`);
      ...
      // Fallback categories
      return [
        `${this.baseUrl}/us/clothing/c/22`,
        `${this.baseUrl}/us/clothing/c/22?filter=dresses`,
        `${this.baseUrl}/us/clothing/c/22?filter=tops`,
        `${this.baseUrl}/us/clothing/c/22?filter=pants`,
        `${this.baseUrl}/us/clothing/c/22?filter=jackets`
      ];

    } catch (error) {
      logger.error('[Dynamite] Error getting category URLs:', error);
      return [
        `${this.baseUrl}/us/clothing/c/22`,
        `${this.baseUrl}/us/clothing/c/22?filter=dresses`,
        `${this.baseUrl}/us/clothing/c/22?filter=tops`
  ...
  async scrapeCategory(page, categoryUrl) {
    ...
    try {
      logger.info(`[Dynamite] Scraping category: ${categoryUrl}`);
      ...
      try {
        ...
        logger.warn('[Dynamite] Network did not idle, continuing anyway');
      }
  ...
  async saveProduct(product) {
    ...
    try {
      ...
      if (product.price) {
        await client.query(`
          INSERT INTO dynamite_price_history (product_id, price, was_on_sale, sale_price, recorded_at)
          VALUES ($1, $2, $3, $4, NOW())
  ...
  async saveSnapshot(date, stats, duration, status = 'success', errorLog = null) {
    try {
      ...
      await client.query(`
        INSERT INTO dynamite_inventory_snapshots (
      ...
      logger.error('[Dynamite] Error saving snapshot:', error);
    }
  }
...
module.exports = new DynamiteInventoryService();
