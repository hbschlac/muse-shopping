/**
 * Macys Inventory Scraping Service
 ...
 *
 * Recommended: Contact Macys for API access or explicit permission for research
 */
...
class MacysInventoryService {
  constructor() {
    this.baseUrl = 'https://www.macys.com';
    this.womensCategoryUrl = `${this.baseUrl}/shop/womens-clothing`;
  ...
  async scrapeInventory() {
    ...
    logger.info('[Macys] Starting inventory scrape');
    ...
    try {
      ...
      logger.info(`[Macys] Found ${categoryUrls.length} category URLs`);
      ...
      for (const categoryUrl of categoryUrls) {
        try {
          ...
          for (const product of products) {
            try {
              ...
              if (stats.totalProducts >= this.maxProductsPerRun) {
                logger.info(`[Macys] Reached max products limit: ${this.maxProductsPerRun}`);
                break;
              }
            } catch (error) {
              stats.errors++;
              stats.errorLog.push(`Product ${product.productId}: ${error.message}`);
              logger.error(`[Macys] Error saving product:`, error);
            }
          }
          ...
          logger.error(`[Macys] Error scraping category ${categoryUrl}:`, error);
        }
      }
      ...
      logger.info(`[Macys] Scrape completed. Products: ${stats.totalProducts}, Errors: ${stats.errors}`);
      ...
      logger.error('[Macys] Fatal scraping error:', error);
  ...
  async getCategoryUrls(page) {
    try {
      ...
      try {
        ...
        logger.warn('[Macys] Product cards did not load in time');
      }
      ...
      logger.info(`[Macys] Found ${urls.length} potential category URLs`);
      ...
      // Fallback categories
      return [
        `${this.baseUrl}/shop/womens-clothing`,
        `${this.baseUrl}/shop/womens-clothing/dresses`,
        `${this.baseUrl}/shop/womens-clothing/tops`,
        `${this.baseUrl}/shop/womens-clothing/pants`,
        `${this.baseUrl}/shop/womens-clothing/jackets-coats`
      ];

    } catch (error) {
      logger.error('[Macys] Error getting category URLs:', error);
      return [
        `${this.baseUrl}/shop/womens-clothing`,
        `${this.baseUrl}/shop/womens-clothing/dresses`,
        `${this.baseUrl}/shop/womens-clothing/tops`
  ...
  async scrapeCategory(page, categoryUrl) {
    ...
    try {
      logger.info(`[Macys] Scraping category: ${categoryUrl}`);
      ...
      try {
        ...
        logger.warn('[Macys] Network did not idle, continuing anyway');
      }
  ...
  async saveProduct(product) {
    ...
    try {
      ...
      if (product.price) {
        await client.query(`
          INSERT INTO macys_price_history (product_id, price, was_on_sale, sale_price, recorded_at)
          VALUES ($1, $2, $3, $4, NOW())
  ...
  async saveSnapshot(date, stats, duration, status = 'success', errorLog = null) {
    try {
      ...
      await client.query(`
        INSERT INTO macys_inventory_snapshots (
      ...
      logger.error('[Macys] Error saving snapshot:', error);
    }
  }
...
module.exports = new MacysInventoryService();
