/**
 * RueLaLa Integration Service
 * Connects RueLaLa inventory to Muse items catalog
 */

const pool = require('../db/pool');
const logger = require('../config/logger');

class RueLaLaIntegrationService {
  /**
   * Sync RueLaLa products to items and product_catalog tables
   * Store ID 128 for RueLaLa
   */
  async syncRueLaLaToItems() {
    logger.info('[RueLaLa Integration] Starting sync to items table');

    const ruelalaStoreId = 128; // RueLaLa store ID

    try {
      // Get all RueLaLa products
      const productsResult = await pool.query(`
        SELECT
          ...
          is_in_stock,
          description
        FROM ruelala_products
        ORDER BY product_name
      `);

      logger.info(`[RueLaLa Integration] Found ${productsResult.rows.length} products to sync`);
      ...
      for (const product of productsResult.rows) {
        ...
        const itemValues = [
          ...
          product.description || `${product.product_name} from RueLaLa`,
      ...
      logger.info(`[RueLaLa Integration] Sync complete: ${itemsCreated} items, ${catalogCreated} catalog entries`);

      return {
        success: true,
        itemsCreated,
        catalogCreated,
        storeId: ruelalaStoreId
      };

    } catch (error) {
      logger.error('[RueLaLa Integration] Sync failed:', error);
      throw error;
    }
  }
  ...
  /**
   * Get RueLaLa items for newsfeed (based on user's followed brands)
   */
  async getRueLaLaItemsForNewsfeed(userId, limit = 20) {
    const query = `
      SELECT
        i.id,
        i.name,
...
module.exports = new RueLaLaIntegrationService();
