/**
 * H&M Inventory Scraping Service
 ...
 *
 * Recommended: Contact H&M for API access or explicit permission for research
 */
...
class HMInventoryService {
  constructor() {
    ...
    this.womensCategoryUrl = 'https://www2.hm.com/en_us/ladies.html';
  ...
  async scrapeInventory() {
    ...
    logger.info('[H&M] Starting inventory scrape');
    ...
    try {
      ...
      logger.info(`[H&M] Found ${categoryUrls.length} category URLs`);
      ...
      for (const categoryUrl of categoryUrls) {
        try {
          ...
          for (const product of products) {
            try {
              ...
              if (stats.totalProducts >= this.maxProductsPerRun) {
                logger.info(`[H&M] Reached max products limit: ${this.maxProductsPerRun}`);
                break;
              }
            } catch (error) {
              stats.errors++;
              stats.errorLog.push(`Product ${product.productId}: ${error.message}`);
              logger.error(`[H&M] Error saving product:`, error);
            }
          }
          ...
          logger.error(`[H&M] Error scraping category ${categoryUrl}:`, error);
        }
      }
      ...
      logger.info(`[H&M] Scrape completed. Products: ${stats.totalProducts}, Errors: ${stats.errors}`);
      ...
      logger.error('[H&M] Fatal scraping error:', error);
  ...
  async getCategoryUrls(page) {
    try {
      ...
      try {
        ...
        logger.warn('[H&M] Product cards did not load in time');
      }
      ...
      const urls = await page.evaluate((baseUrl) => {
        ...
        selectors.forEach(selector => {
          ...
          elements.forEach(link => {
            ...
            if (href && href.includes('ladies') && !links.includes(href)) {
      ...
      logger.info(`[H&M] Found ${urls.length} potential category URLs`);
      ...
      // Fallback categories
      return [
        'https://www2.hm.com/en_us/ladies.html',
        'https://www2.hm.com/en_us/ladies/clothing.html',
        'https://www2.hm.com/en_us/ladies/clothing/dresses.html',
        'https://www2.hm.com/en_us/ladies/clothing/tops.html',
        'https://www2.hm.com/en_us/ladies/clothing/pants.html'
      ];

    } catch (error) {
      logger.error('[H&M] Error getting category URLs:', error);
      return [
        'https://www2.hm.com/en_us/ladies.html',
        'https://www2.hm.com/en_us/ladies/clothing.html',
        'https://www2.hm.com/en_us/ladies/clothing/dresses.html'
  ...
  async scrapeCategory(page, categoryUrl) {
    ...
    try {
      logger.info(`[H&M] Scraping category: ${categoryUrl}`);
      ...
      try {
        ...
        logger.warn('[H&M] Network did not idle, continuing anyway');
      }
  ...
  async saveProduct(product) {
    ...
    try {
      ...
      if (product.price) {
        await client.query(`
          INSERT INTO hm_price_history (product_id, price, was_on_sale, sale_price, recorded_at)
          VALUES ($1, $2, $3, $4, NOW())
  ...
  async saveSnapshot(date, stats, duration, status = 'success', errorLog = null) {
    try {
      ...
      await client.query(`
        INSERT INTO hm_inventory_snapshots (
      ...
      logger.error('[H&M] Error saving snapshot:', error);
    }
  }
...
module.exports = new HMInventoryService();
