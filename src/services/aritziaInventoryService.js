/**
 * Aritzia Inventory Scraping Service
 ...
 *
 * Recommended: Contact Aritzia for API access or explicit permission for research
 */
...
class AritziaInventoryService {
  constructor() {
    this.baseUrl = 'https://www.aritzia.com';
    this.womensCategoryUrl = `${this.baseUrl}/us/en/clothing`;
  ...
  async scrapeInventory() {
    ...
    logger.info('[Aritzia] Starting inventory scrape');
    ...
    try {
      ...
      logger.info(`[Aritzia] Found ${categoryUrls.length} category URLs`);
      ...
      for (const categoryUrl of categoryUrls) {
        try {
          ...
          for (const product of products) {
            try {
              ...
              if (stats.totalProducts >= this.maxProductsPerRun) {
                logger.info(`[Aritzia] Reached max products limit: ${this.maxProductsPerRun}`);
                break;
              }
            } catch (error) {
              stats.errors++;
              stats.errorLog.push(`Product ${product.productId}: ${error.message}`);
              logger.error(`[Aritzia] Error saving product:`, error);
            }
          }
          ...
          logger.error(`[Aritzia] Error scraping category ${categoryUrl}:`, error);
        }
      }
      ...
      logger.info(`[Aritzia] Scrape completed. Products: ${stats.totalProducts}, Errors: ${stats.errors}`);
      ...
      logger.error('[Aritzia] Fatal scraping error:', error);
  ...
  async getCategoryUrls(page) {
    try {
      ...
      try {
        ...
        logger.warn('[Aritzia] Product cards did not load in time');
      }

      const urls = await page.evaluate((baseUrl) => {
        const links = [];
        const selectors = [
          'a[href*="/us/en/clothing"]',
          'a[href*="/clothing"]',
          'nav a[href*="clothing"]',
          '.category-link'
        ];
      ...
      logger.info(`[Aritzia] Found ${urls.length} potential category URLs`);
      ...
      // Fallback categories
      return [
        `${this.baseUrl}/us/en/clothing`,
        `${this.baseUrl}/us/en/clothing/dresses`,
        `${this.baseUrl}/us/en/clothing/tops`,
        `${this.baseUrl}/us/en/clothing/pants`,
        `${this.baseUrl}/us/en/clothing/jackets-coats`
      ];

    } catch (error) {
      logger.error('[Aritzia] Error getting category URLs:', error);
      return [
        `${this.baseUrl}/us/en/clothing`,
        `${this.baseUrl}/us/en/clothing/dresses`,
        `${this.baseUrl}/us/en/clothing/tops`
  ...
  async scrapeCategory(page, categoryUrl) {
    ...
    try {
      logger.info(`[Aritzia] Scraping category: ${categoryUrl}`);
      ...
      try {
        ...
        logger.warn('[Aritzia] Network did not idle, continuing anyway');
      }
  ...
  async saveProduct(product) {
    ...
    try {
      ...
      if (product.price) {
        await client.query(`
          INSERT INTO aritzia_price_history (product_id, price, was_on_sale, sale_price, recorded_at)
          VALUES ($1, $2, $3, $4, NOW())
  ...
  async saveSnapshot(date, stats, duration, status = 'success', errorLog = null) {
    try {
      ...
      await client.query(`
        INSERT INTO aritzia_inventory_snapshots (
      ...
      logger.error('[Aritzia] Error saving snapshot:', error);
    }
  }
...
module.exports = new AritziaInventoryService();
