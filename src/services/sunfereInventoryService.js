/**
 * Sunfere Inventory Scraping Service
 * Purpose: Academic research - tracking Sunfere fashion inventory
 *
 * IMPORTANT: This service is designed for academic research purposes.
 * Users must ensure compliance with:
 * - Sunfere's Terms of Service
 * - Sunfere's robots.txt
 * - Applicable data protection regulations
 * - Academic institution's research ethics guidelines
 *
 * Recommended: Contact Sunfere for API access or explicit permission for research
 */
...
class SunfereInventoryService {
  constructor() {
    this.baseUrl = 'https://sunfere.com';
  ...
  async scrapeInventory() {
    ...
    logger.info('[Sunfere] Starting inventory scrape');
    ...
    try {
      ...
      logger.info(`[Sunfere] Found ${collectionUrls.length} collection URLs`);
      ...
      for (const collectionUrl of collectionUrls) {
        try {
          ...
          for (const product of products) {
            try {
              ...
              if (stats.totalProducts >= this.maxProductsPerRun) {
                logger.info(`[Sunfere] Reached max products limit: ${this.maxProductsPerRun}`);
                break;
              }
            } catch (error) {
              stats.errors++;
              stats.errorLog.push(`Product ${product.productId}: ${error.message}`);
              logger.error(`[Sunfere] Error saving product:`, error);
            }
          }
          ...
          logger.error(`[Sunfere] Error scraping collection ${collectionUrl}:`, error);
        }
      }
      ...
      logger.info(`[Sunfere] Scrape completed. Products: ${stats.totalProducts}, Errors: ${stats.errors}`);
      ...
      logger.error('[Sunfere] Fatal scraping error:', error);
  ...
  async getCollectionUrls(page) {
    try {
      ...
      try {
        ...
        logger.warn('[Sunfere] Product cards did not load in time');
      }
      ...
      logger.info(`[Sunfere] Found ${urls.length} potential collection URLs`);
      ...
      logger.error('[Sunfere] Error getting collection URLs:', error);
      // Return fallback
      return [this.shopUrl];
    }
  }
  ...
  async scrapeCollection(page, collectionUrl) {
    ...
    try {
      logger.info(`[Sunfere] Scraping collection: ${collectionUrl}`);
      ...
      try {
        ...
        logger.warn('[Sunfere] Network did not idle, continuing anyway');
      }
      ...
      logger.info(`[Sunfere] Page info: ${JSON.stringify(pageInfo)}`);
      ...
      const productData = await page.evaluate(() => {
        ...
        productCards.forEach((card, index) => {
          try {
            ...
            // Extract brand (Sunfere may not have explicit brand fields)
            const brandSelectors = ['.brand', '.vendor', '[itemprop="brand"]', 'span.brand'];
            let brandName = 'Sunfere'; // Default brand
            ...
            if (imageUrl && !imageUrl.startsWith('http')) {
              imageUrl = imageUrl.startsWith('//') ? 'https:' + imageUrl : 'https://sunfere.com' + imageUrl;
            }
            ...
            if (productUrl && !productUrl.startsWith('http')) {
              productUrl = 'https://sunfere.com' + productUrl;
            }
  ...
  async saveProduct(product) {
    ...
    try {
      ...
      if (product.price) {
        await client.query(`
          INSERT INTO sunfere_price_history (product_id, price, recorded_at)
          VALUES ($1, $2, NOW())
  ...
  async saveSnapshot(date, stats, duration, status = 'success', errorLog = null) {
    try {
      ...
      await client.query(`
        INSERT INTO sunfere_inventory_snapshots (
      ...
      logger.error('[Sunfere] Error saving snapshot:', error);
    }
  }
...
module.exports = new SunfereInventoryService();
