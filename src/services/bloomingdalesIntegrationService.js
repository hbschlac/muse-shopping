/**
 * Bloomingdales Integration Service
...
class BloomingdalesIntegrationService {
  /**
   * Sync Bloomingdales products to items and product_catalog tables
   * Store ID 11 for Bloomingdales
   */
  async syncBloomingdalesToItems() {
    logger.info('[Bloomingdales Integration] Starting sync to items table');

    const bloomingdalesStoreId = 11; // Bloomingdales store ID

    try {
      // Get all Bloomingdales products
      ...
      logger.info(`[Bloomingdales Integration] Found ${productsResult.rows.length} products to sync`);
      ...
      for (const product of productsResult.rows) {
        ...
        const itemValues = [
          bloomingdalesStoreId,
          product.product_id,
          brandId,
          product.product_name,
          product.product_name, // canonical_name
          product.description || `${product.product_name} from Bloomingdales`,
      ...
      logger.info(`[Bloomingdales Integration] Sync complete: ${itemsCreated} items, ${catalogCreated} catalog entries`);

      return {
        success: true,
        itemsCreated,
        catalogCreated,
        storeId: bloomingdalesStoreId
      };

    } catch (error) {
      logger.error('[Bloomingdales Integration] Sync failed:', error);
      throw error;
    }
  }
  ...
  /**
   * Get Bloomingdales items for newsfeed (based on user's followed brands)
   */
  async getBloomingdalesItemsForNewsfeed(userId, limit = 20) {
    const query = `
      SELECT
        i.id,
        i.name,
...
module.exports = new BloomingdalesIntegrationService();
